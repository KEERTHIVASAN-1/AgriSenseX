...
export default function FloatingChat() {
  ...
  const [messages, setMessages] = useState<Array<{ id: number; text: string; sender: "user" | "bot"; jsonData?: any }>>([
    { id: 1, text: "Hello! I'm your Farm Assistant and Plant Disease Expert. I can help you with:\n• Plant disease diagnosis and treatment\n• Farming tips and best practices\n• Soil and land management\n• Crop management advice\n• Irrigation and water management\n• And much more!\n\nAsk me anything about farming or describe plant symptoms for analysis.", sender: "bot" },
  ]);
          ...
          <div className="flex items-center justify-between p-4 sm:p-6 bg-gradient-to-r from-[#7faf3b] to-[#6a9331] text-white shadow-md">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-white/80">Online - Expert Advice</p>
              </div>
            </div>
            <button
                  ...
                  >
                    {message.sender === "bot" && message.jsonData ? (
                      <div className="space-y-3">
                        {message.jsonData.queryType === "disease" ? (
                          // Disease-related query display
                          <>
                            <div className="border-b border-gray-200 pb-2 mb-3">
                              <h4 className="font-semibold text-lg text-[#7faf3b] mb-1">
                                {message.jsonData.diseaseName && message.jsonData.diseaseName !== "Unknown" 
                                  ? message.jsonData.diseaseName 
                                  : "Disease Analysis"}
                              </h4>
                              <div className="flex flex-wrap gap-2 text-xs">
                                {message.jsonData.confidence && (
                          ...
                          // General farming query display
                          <>
                            <div className="border-b border-gray-200 pb-2 mb-3">
                              <h4 className="font-semibold text-lg text-[#7faf3b] mb-1">
                                {message.jsonData.topic || "Farming Advice"}
                              </h4>
                            </div>
                            
                            {message.jsonData.answer && (
                              <div>
                                <h5 className="font-semibold text-sm mb-1">Answer:</h5>
                                <p className="text-sm text-gray-700">{message.jsonData.answer}</p>
                              </div>
                            )}
                            
                            {message.jsonData.tips && message.jsonData.tips.length > 0 && (
                              <div>
                                <h5 className="font-semibold text-sm mb-1 text-[#7faf3b]">💡 Tips & Tricks:</h5>
                                <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                                  {message.jsonData.tips.map((tip: string, idx: number) => (
                                    <li key={idx}>{tip}</li>
                                  ))}
                                </ul>
                              </div>
                            ...
                            {message.jsonData.bestPractices && message.jsonData.bestPractices.length > 0 && (
                              <div>
                                <h5 className="font-semibold text-sm mb-1 text-[#7faf3b]">✅ Best Practices:</h5>
                                <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                                  {message.jsonData.bestPractices.map((practice: string, idx: number) => (
                            ...
                            {message.jsonData.commonMistakes && message.jsonData.commonMistakes.length > 0 && (
                              <div>
                                <h5 className="font-semibold text-sm mb-1 text-orange-600">⚠️ Common Mistakes to Avoid:</h5>
                                <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                                  {message.jsonData.commonMistakes.map((mistake: string, idx: number) => (
                            ...
                            {message.jsonData.seasonalAdvice && (
                              <div className="bg-blue-50 p-3 rounded-lg">
                                <h5 className="font-semibold text-sm mb-1 text-blue-700">🌾 Seasonal Advice:</h5>
                                <p className="text-sm text-gray-700">{message.jsonData.seasonalAdvice}</p>
                              </div>
                            )}
                            
                            {message.jsonData.toolsOrResources && message.jsonData.toolsOrResources.length > 0 && (
                              <div>
                                <h5 className="font-semibold text-sm mb-1 text-[#7faf3b]">🛠️ Tools & Resources:</h5>
                                <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                                  {message.jsonData.toolsOrResources.map((resource: string, idx: number) => (
